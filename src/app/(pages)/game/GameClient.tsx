...
export default function GameClient({ initialData }: { initialData: any }) {
  ...
  const categories = useMemo(() => {
    ...
    (games || []).forEach((g: any) => {
      const c = col(g, 'カテゴリー') || col(g, 'ai') || g['カテゴリー'];
      if (c) cats.add(c);
    });
    ...
    <div>
      ...
      <div className="glass-panel" style={{ padding: '20px', marginBottom: '24px', display: 'flex', flexWrap: 'wrap', gap: '20px', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          >
            {sortedGames
              .filter((g: any) => selectedCategory === '全カテゴリー' || (col(g, 'カテゴリー') || col(g, 'ai') || g['カテゴリー']) === selectedCategory)
              .map((g: any) => (
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <select 
            ...
            onChange={e => {
              ...
              const newGames = sortedGames.filter((g: any) => e.target.value === '全カテゴリー' || (col(g, 'カテゴリー') || col(g, 'ai') || g['カテゴリー']) === e.target.value);
